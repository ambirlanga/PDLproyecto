...
%type<lis> listaParametrosFormales listaCampos parametrosFormales
...
programa 	    : {dvar=0; niv = 0; si=0; cargaContexto(niv);
            ...
            listaDeclaraciones
                ...
                $<refe>$.ref3 = sim.d;
                completaLans($<refe>1.ref2, crArgEtq($<refe>$.ref3));
            }
...
declaracionFuncion  : tipoSimple ID_ {$<cent>$=dvar; dvar = 0; niv=1; cargaContexto(niv);} APAR_ parametrosFormales CPAR_ 
                    {$<cent>$=0;
                    if(!insTdS($2, FUNCION, $1, 0, si, $5.ref)){yyerror("Identificador de funcion repetido");
                    ...
                    if (strcmp($2,"main") == 0) { emite(FIN, crArgNul(), crArgNul(), crArgNul()); $$ = -1;}
                    else { emite(RET, crArgNul(), crArgNul(), crArgNul()); $$ = 0;}
...
parametrosFormales  : { $$.ref = insTdD(-1,T_VACIO); $$.talla = 0;}     
                    | listaParametrosFormales { $$.ref = $1.ref; $$.talla = $1.talla; }
                    ;
                        ...
                        if ($5.tipo != T_ERROR){
                            if (sim.t != T_RECORD) {yyerror("El identificador debe ser struct");}   
                                ...
                                if (reg.t == T_ERROR) {yyerror("Campo no declarado");}   
                                    ...
                                    emite(EASIG, crArgPos(niv, $5.d),  crArgNul(), crArgPos(sim.n, reg.d)); 
                                }   // REVISAR
                            }
                        } 
                        }
...
instruccionIteracion: WHILE_ APAR_ { $<refe>$.ref1 = si;} expresion CPAR_ 
                    {
                    if($4.tipo != T_ERROR && $4.tipo != T_LOGICO) {yyerror("La expresion del while debe ser logica");}
                    
                    $<refe>$.ref1 = creaLans(si);   // REVISAR
                    
                   
                    emite(EIGUAL, crArgPos(niv, $4.d), crArgEnt(1), crArgEtq(-1) );

                    $<refe>$.ref2 = creaLans(si);                              
                    emite(GOTOS, crArgNul(), crArgNul(), crArgEtq(-1));
                    $<refe>$.ref3 = si; 

                    emite(GOTOS, crArgNul(), crArgNul(), crArgEtq($<refe>3.ref1)); 
                    completaLans($<refe>$.ref1, crArgEtq(si));
                    }
                    instruccion 
                    {
                    emite(GOTOS, crArgNul(), crArgNul(), crArgEtq($<refe>6.ref3));
                    completaLans($<refe>6.ref2, crArgEtq(si));
                    }
                    ;
/********************************************************************************************************/  
...
expresionSufija     : constante {$$.tipo = $1.tipo; $$.d = creaVarTemp(); emite(EASIG, crArgEnt($1.d), crArgNul(), crArgPos(niv, $$.d)); }
            ...
            {   
                ...
                if (sim.t != T_RECORD) {yyerror("El identificador debe ser struct");}   
                    ...
                    if (reg.t == T_ERROR) {yyerror("Campo no declarado");}   
                        ...
                        $$.tipo  = reg.t;
                        $$.d = creaVarTemp();
                        emite(EASIG, crArgPos(sim.n, reg.d),  crArgNul(), crArgPos(niv, $$.d));  // REVISAR
                    }
                }                
            }
            ...
            {       
                ...
                emite(EAV, crArgPos(sim.n, sim.d), crArgPos(niv, $3.d), crArgPos(niv, $$.d));                             
                    ...
                    | ID_ {emite(EPUSH, crArgNul(), crArgNul(), crArgEnt(0));} APAR_ parametrosActuales CPAR_
            {
...
listaParametrosActuales: expresion 
                    ...
                    | expresion COMA_ {emite(EPUSH,crArgNul(),crArgNul(),crArgPos(niv, $1.d));} listaParametrosActuales 
                    { $$ = insTdD($4,$1.tipo);}
                    ;
